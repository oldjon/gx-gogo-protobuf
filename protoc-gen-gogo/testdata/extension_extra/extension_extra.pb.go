...
func (m *ExtraMessage) Scan(src interface{}) error {
	if p == nil {
		return errors.New("ExtraMessage is nil")
	}
	var source []byte
	switch v := src.(type) {
	case []byte:
	...
	case nil:
		return nil
	default:
		return fmt.Errorf("incompatible type for ExtraMessage %T", v)
	}
	return m.Unmarshal(source)
}
func (m *ExtraMessage) Value() (driver.Value, error) {
	return m.Marshal()
}
