...
func (m *Local) Scan(src interface{}) error {
	if p == nil {
		return errors.New("Local is nil")
	}
	var source []byte
	switch v := src.(type) {
	case []byte:
	...
	case nil:
		return nil
	default:
		return fmt.Errorf("incompatible type for Local %T", v)
	}
	return m.Unmarshal(source)
}
func (m *Local) Value() (driver.Value, error) {
	return m.Marshal()
}
